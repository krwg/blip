...
export function startPresenceLoop({
  ...
  const tick = async () => {
    ...
    const payload = {
      ...
      callPingMs: callFresh?.rttMs ?? callFresh?.pingMs ?? null,
      callJitterMs: callFresh?.jitterMs ?? null,
      callQualityTier: callFresh?.qualityTier || '',
      callPacketLoss: callFresh?.packetLoss ?? null,
