...
export function configureAutoUpdater(config) {
  ...
  autoUpdater.allowPrerelease = !!config?.receiveBetaUpdates;
...
export function setupAutoUpdater(getWindow, getConfig) {
  ...
  attachListeners();
  configureAutoUpdater(typeof getConfig === 'function' ? getConfig() : null);
...
export async function checkForUpdatesNow(getConfig) {
  ...
  attachListeners();
  if (typeof getConfig === 'function') configureAutoUpdater(getConfig());
  try {
