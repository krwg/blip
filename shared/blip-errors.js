...
export const BLIP_ERROR_CATALOG = Object.freeze({
    ...
    104: {
        ...
        detail: 'Legacy umbrella. Prefer 117–129. TCP closed while a handshake waiter was pending without a tagged reason.',
    },
    105: {
    ...
    109: {
        ...
        detail: 'Peer closed TCP during/after mesh-handshake. Morse retries plaintext compat when allowed.',
    },
    110: {
        id: 'COMPAT_PLAINTEXT',
...
export function isSocketCloseFamily(code) {
    ...
    if (!Number.isFinite(n))
        return false;
    return (n === BlipErrorCode.SOCKET_CLOSED ||
...
export function destroySocketTagged(socket, code, detail = '') {
    ...
    console.error(`[BLIP E${code}/${BLIP_ERROR_CATALOG[code]?.id || '?'}] destroy: ${detail || BLIP_ERROR_CATALOG[code]?.summary || ''}`);
    try {
        socket.destroy();
    }
    catch {
...
export function classifyBlipError(err) {
    if (err && typeof err.blipCode === 'number')
        return err;
...
export function logBlipError(err, context = '') {
    ...
    if (doc?.detail)
        console.error(`  detail: ${doc.detail}`);
    if (classified.cause) {
